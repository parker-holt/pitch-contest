...
import { getDocs, query, orderBy } from '@/lib/firebase'
        ...
        <div style={card}>
          ...
          {judges.length > 0 && (
            <div style={{ borderTop: '1px solid var(--border)', marginTop: 4, paddingTop: 12 }}>
              <div style={{ fontSize: 11, fontWeight: 600, color: 'var(--tl)', marginBottom: 8 }}>Judge portal links (private)</div>
              {judges.map(j => {
          ...
          {judges.length === 0 && (
            <p style={{ fontSize: 13, color: 'var(--tl)', marginTop: 8 }}>Visit /api/contest first to seed judges.</p>
          )}
        </div>
            ...
            <tbody>
              {subs.map(s => (
                <tr key={s.id}>
                  ...
                  <td style={{ padding: '10px', borderBottom: '1px solid #f0f2f5', color: 'var(--tm)' }}>{s.teamName}</td>
